...
- Short UFCS accepts receiver types known before checker execution: function
  ...
  variable-assignment, variable-evaluation, place-level, state/value/path-update,
  restore-path, and consume-path reverse hidden-frame lifts for the expression
  replay proof. Hidden
...
1. Finish UFCS receiver hardening.
   ...
   - Direct-call receivers are not an OCaml-only switch: existing store-safe
     ...
     The next proof step is proving the remaining place-mutation/control-flow
