...
- The active endpoint can currently be proved with either an explicit
  ...
  over the proved branches exist; the no-receiver runtime path and case
  dispatcher now consume the packaged ready-body route provider. Consuming the concrete paired evidence in
...
2. Finish direct-call receiver activation.
   - Prove runtime soundness for the proposed gate
     ...
     path and case dispatcher consuming the packaged ready-body route provider
     are proved; the next step is proving or
...
- Direct-call receiver safety-gate tests remain invalid. Diagnostics for the
  ...
  proved under the explicit direct-ready gate. A case-dispatched
