...
- A mixed assoc direct-receiver endpoint,
  ...
  first core cleanup route, the exact-body cleanup core and package-at routes,
  and the first exact-body route-package layers, including per-function,
  reachable package-provider, and reachable package-and-target-provider
  variants, expose prefix-callback theorems while keeping legacy public shapes
  as wrappers. Higher route wrappers still bridge legacy static-runtime
  premises into the prefix chain.
...
- A trial CLI switch to
  ...
  and package callback chain to expose the prefix-aware callback shape directly.
