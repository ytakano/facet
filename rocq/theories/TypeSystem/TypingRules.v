...
Inductive ty_compatible (Ω : outlives_ctx) : Ty -> Ty -> Prop :=
  ...
  | TC_Ref_Shared : forall ua ue la lb Ta Tb,
      usage_sub ua ue ->
      outlives Ω la lb ->
      ty_compatible Ω Ta Tb ->
      ty_compatible Ω (MkTy ua (TRef la RShared Ta)) (MkTy ue (TRef lb RShared Tb))
  | TC_Ref_Unique : forall ua ue la lb Ta,
      usage_sub ua ue ->
      outlives Ω la lb ->
      ty_compatible Ω (MkTy ua (TRef la RUnique Ta)) (MkTy ue (TRef lb RUnique Ta))
  ...
  | TC_Closure : forall ua ue la lb params_a params_e ret_a ret_e,
      usage_sub ua ue ->
      outlives Ω la lb ->
      Forall2 (fun expected actual => ty_compatible Ω expected actual) params_e params_a ->
      ty_compatible Ω ret_a ret_e ->
      ty_compatible Ω
        (MkTy ua (TClosure la params_a ret_a))
        (MkTy ue (TClosure lb params_e ret_e))
  | TC_Fn_Closure : forall ua ue lb params_a params_e ret_a ret_e,
      usage_sub ua ue ->
      Forall2 (fun expected actual => ty_compatible Ω expected actual) params_e params_a ->
      ty_compatible Ω ret_a ret_e ->
      ty_compatible Ω
...
Inductive typed (fenv : list fn_def) (Ω : outlives_ctx) (n : nat) : ctx -> expr -> Ty -> ctx -> Prop :=
  ...
  | T_CallExpr_Forall_Closure :
      forall Γ Γ1 Γ' callee args u m bounds body env_lt param_tys ret σ,
      typed fenv Ω n Γ callee (MkTy u (TForall m bounds body)) Γ1 ->
