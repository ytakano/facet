...
2. **M1: Rocq core type/syntax/env 拡張** [done]
...
4. **M3: TypeChecker の struct/trait 環境対応** [done]
...
5. **M4: field place / partial move / partial reference** [done]
...
6. **M5: OCaml surface syntax 実装** [done]
...
8. **M7: Soundness proofs 拡張** [done]
   - `theories/Validation/ValidatorSoundness.v` を追加し、Validator が返す環境の well-formedness を証明済み。
   - `OperationalSemantics.v` は `global_env` ベースの struct-aware semantics に更新済み。`VRef` は root と field path を保持し、field move/replace/borrow は path-aware store state を扱う。
   - `EnvStructuralRules.v` に struct-aware env checker の wrapper-free Prop surface と、struct literal、field place、field replace/assign、path borrow の構造的規則を追加済み。
   - `EnvTypingSoundness.v` / `EnvBorrowSoundness.v` / `EnvFullSoundness.v` で env/stateful checker の structural soundness を証明済み。
   - `CheckerSoundness.v` / `CheckerUsageSoundness.v` / `BorrowCheckSoundness.v` の legacy checker 系は既存互換を維持。旧 `infer_body` / `borrow_check` の struct/field `ErrNotImplemented` 解消は別タスク。

9. **M8: regression tests と docs** [done]
...
- moved field を replace して restored path が消え、全 field が available になった場合は parent reference を再許可する。
- FIR は field path を `fir_place` に保持し、nested field の conflict/debug 情報を保つ。
- generic impl matching は型変数・lifetime 変数への一階マッチングとし、複数候補一致は ambiguous impl error にする。
