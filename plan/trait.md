...
- Short UFCS accepts receiver types known before checker execution: function
  ...
  evaluation, cleanup, and inversion helpers, structured receiver-method checker view lemmas, direct and generic typed
  hidden-body packages for receiver-method summaries, direct and generic replay-parameterized hidden-let
  evaluation bridges, a method-argument hidden-receiver strip helper, a hidden receiver method-call inversion helper, direct and generic checked hidden-body packages, direct and generic conditional hidden-body eval safety helpers, direct and generic hidden-let eval inversion helpers, and a behavior-preserving split
...
1. Finish UFCS receiver hardening.
   ...
   - Direct-call receivers are not an OCaml-only switch: existing store-safe
     ...
     constructors, hidden receiver `EVar` evaluation, cleanup, and inversion helpers, hidden-let
     receiver-method checker summaries, structured checker view lemmas, direct and generic typed
     hidden-body packages for receiver-method summaries, direct and generic replay-parameterized hidden-let evaluation bridges, a method-argument hidden-receiver strip helper, a hidden receiver method-call inversion helper, direct and generic checked hidden-body packages, direct and generic conditional hidden-body eval safety helpers, direct and generic hidden-let eval inversion helpers, and the active-core/public-base gate split
