...
- Runtime proof plumbing now has prefix-aware static-runtime callback shapes,
  ...
  wrapper chain.
  The remaining runtime theorem gap is deriving a concrete exact-closure local-bounds route callback, now wired through the mixed endpoint, from the public/static completeness chain without adding a premise, then retargeting the required public theorem.
