...
(** val find_impl_assoc :
    string -> impl_assoc_def list -> impl_assoc_def option **)

let rec find_impl_assoc assoc_name = function
| [] -> None
| a :: rest ->
  if (=) a.impl_assoc_name assoc_name
  then Some a
  else find_impl_assoc assoc_name rest

(** val normalize_assoc_ty_fuel :
    Big_int_Z.big_int -> global_env -> ty -> ty **)

let rec normalize_assoc_ty_fuel fuel env t =
  (fun fO fS n -> if Big_int_Z.sign_big_int n <= 0 then fO ()
  else fS (Big_int_Z.pred_big_int n))
    (fun _ -> t)
    (fun fuel' ->
    ...
    (match t0 with
     ...
     | TFn (params, ret) -> MkTy (u, (TFn ((map norm params), (norm ret))))
     | TClosure (lt, params, ret) ->
       MkTy (u, (TClosure (lt, (map norm params), (norm ret))))
     | TForall (n, bounds, body) ->
     ...
     | TAssoc (for_ty, trait_name0, trait_args, assoc_name) ->
       ...
       (match matching_impls trait_name0 trait_args' for_ty' env.env_impls with
        | [] ->
          MkTy (u, (TAssoc (for_ty', trait_name0, trait_args', assoc_name)))
        | impl :: l ->
          (match l with
           | [] ->
             (match find_impl_assoc assoc_name impl.impl_assoc_types with
              | Some assoc ->
                let normalized = norm assoc.impl_assoc_ty in
                MkTy (u, (ty_core normalized))
              | None ->
                MkTy (u, (TAssoc (for_ty', trait_name0, trait_args',
                  assoc_name))))
           | _ :: _ ->
             MkTy (u, (TAssoc (for_ty', trait_name0, trait_args',
               assoc_name)))))
...
let normalize_assoc_ty env t =
  normalize_assoc_ty_fuel (Big_int_Z.succ_big_int (Big_int_Z.succ_big_int
    (Big_int_Z.succ_big_int (Big_int_Z.succ_big_int (Big_int_Z.succ_big_int
    (Big_int_Z.succ_big_int (Big_int_Z.succ_big_int (Big_int_Z.succ_big_int
    (Big_int_Z.succ_big_int (Big_int_Z.succ_big_int (Big_int_Z.succ_big_int
    (Big_int_Z.succ_big_int (Big_int_Z.succ_big_int (Big_int_Z.succ_big_int
...
let generic_direct_call_receiver_method_target_expr = function
| ECallGeneric (method_name, type_args, l) ->
  (match l with
   | [] -> None
   | e0 :: method_args ->
...
let check_fn_root_shadow_generic_direct_store_safe_summary_target env fdef fname type_args args synthetic_body =
  (&&) (store_safe_function_value_call_args_b env args)
    (match lookup_fn_b fname env.env_fns with
     | Some callee ->
                           ...
                           ((&&)
                             ((&&)
                               ((&&)
                                 ...
                                 (ty_compatible_b callee.fn_outlives t_callee
                                   (subst_type_params_ty type_args
                                     callee.fn_ret)))
                               (fn_params_roots_exclude_b
                                 (apply_type_params type_args
                                 ...
                                 roots_callee))
                             (fn_params_root_env_excludes_b
                               (apply_type_params type_args callee.fn_params)
                               r_callee))
                           (ty_compatible_b fdef.fn_outlives t_body
                             fdef.fn_ret))
                         (fn_params_roots_exclude_b fdef.fn_params roots))
...
let check_fn_root_shadow_generic_direct_store_safe_summary env fdef =
  ...
  | Some p ->
    ...
    check_fn_root_shadow_generic_direct_store_safe_summary_target env fdef
...
(** val check_fn_root_shadow_no_capture_direct_call_component_store_safe_summary_with_ready_body_summary :
    global_env -> fn_def -> bool **)

let check_fn_root_shadow_no_capture_direct_call_component_store_safe_summary_with_ready_body_summary env fdef =
  if check_fn_root_shadow_no_capture_direct_call_component_store_safe_summary
       env fdef
...
let check_env_root_shadow_no_capture_direct_call_component_store_safe_summary_with_ready_body_summary env =
  forallb
...
let check_fn_root_shadow_no_capture_direct_call_component_store_safe_summary_with_local_bounds_narrow_summary env fdef =
  if check_fn_root_shadow_no_capture_direct_call_component_store_safe_summary
       env fdef
  then check_env_root_shadow_store_safe_narrow_summary
         (global_env_with_local_bounds env fdef.fn_bounds)
  else true
...
let check_env_root_shadow_no_capture_direct_call_component_store_safe_summary_with_local_bounds_narrow_summary env =
  forallb
...
let check_fn_root_shadow_direct_receiver_method_or_no_capture_direct_component_store_safe_summary env fdef =
  (||)
...
let check_env_root_shadow_direct_receiver_method_or_no_capture_direct_component_store_safe_summary env =
  forallb
...
(** val check_fn_root_shadow_captured_call_core_store_safe_summary :
    global_env -> fn_def -> bool **)

let check_fn_root_shadow_captured_call_core_store_safe_summary env fdef =
  (||)
    ((||)
      ((||)
        ((||)
...
let check_fn_root_shadow_captured_call_base_store_safe_summary =
  check_fn_root_shadow_captured_call_core_store_safe_summary
...
let infer_fn_env_end2end_assoc_strict_exact_closure env f =
  let r0 = initial_root_env_for_params (app f.fn_params f.fn_captures) in
  (match infer_full_env_roots_checked_assoc env f r0 with
   | Infer_ok res ->
     if check_fn_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary
...
let rec infer_fns_env_end2end_assoc_strict_exact_closure env = function
| [] -> Infer_ok ()
| f :: rest ->
...
let infer_program_env_end2end_assoc_strict_exact_closure env =
  let env_alpha = alpha_normalize_global_env env in
  if global_names_unique_b env_alpha
  then (match infer_program_env_alpha_elab env with
        | Infer_ok env_elab ->
          (match infer_fns_env_end2end_assoc_strict_exact_closure env_elab
                   env_elab.env_fns with
           | Infer_ok _ -> Infer_ok env_elab
           | Infer_err err -> Infer_err err)
        | Infer_err err -> Infer_err err)
...
(** val check_program_env_end2end_assoc_strict_exact_closure :
    global_env -> bool **)
...
let check_fn_root_shadow_direct_receiver_method_present _ fdef =
  match direct_call_receiver_method_target_expr fdef.fn_body with
  | Some _ -> true
  | None ->
    (match generic_direct_call_receiver_method_target_expr fdef.fn_body with
     | Some _ -> true
     | None -> false)

(** val check_env_root_shadow_direct_receiver_method_present :
    global_env -> bool **)

let check_env_root_shadow_direct_receiver_method_present env =
...
let check_env_root_shadow_no_receiver_component_body_summary_provider_check env =
...
let check_env_root_shadow_no_receiver_component_ready_body_summary_provider_check env =
  (||) (check_env_root_shadow_direct_receiver_method_present env)
...
let check_env_root_shadow_no_receiver_component_ready_body_summary_provider_check_with_shadow_checks env =
  (||) (check_env_root_shadow_direct_receiver_method_present env)
    ((&&)
      ((&&)
        (check_env_root_shadow_no_capture_direct_call_component_store_safe_summary_with_ready_body_summary
...
let check_env_root_shadow_no_receiver_component_ready_body_or_narrow_summary_provider_check env =
  (||)
...
let check_env_root_shadow_no_receiver_component_ready_body_or_local_bounds_narrow_summary_provider_check env =
  (||)
    (check_env_root_shadow_no_receiver_component_ready_body_summary_provider_check
      env)
...
(** val check_program_env_end2end_assoc_direct_receiver_mixed :
    global_env -> bool **)

let check_program_env_end2end_assoc_direct_receiver_mixed env =
...
let infer_program_env_end2end_assoc_direct_receiver_base_mixed env =
  match infer_program_env_end2end_assoc_direct_receiver_base env with
  | Infer_ok env' ->
    if check_env_end2end_direct_receiver_mixed_ready env'
    then Infer_ok env'
...
let check_program_env_end2end_assoc_direct_receiver_base_mixed env =
  match infer_program_env_end2end_assoc_direct_receiver_base_mixed env with
  | Infer_ok _ -> true
  | Infer_err _ -> false
...
let infer_program_env_end2end_assoc_direct_receiver_base_direct_component env =
  match infer_program_env_end2end_assoc_direct_receiver_base env with
  | Infer_ok env' ->
...
let check_program_env_end2end_assoc_direct_receiver_base_direct_component env =
  match infer_program_env_end2end_assoc_direct_receiver_base_direct_component
          env with
  | Infer_ok _ -> true
  | Infer_err _ -> false
...
let infer_program_env_end2end_assoc_strict_exact_closure_direct_receiver_mixed env =
  match infer_program_env_end2end_assoc_strict_exact_closure env with
  | Infer_ok env' ->
    if check_env_end2end_direct_receiver_mixed_ready env'
    then Infer_ok env'
...
let check_program_env_end2end_assoc_strict_exact_closure_direct_receiver_mixed env =
