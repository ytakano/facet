...
- Short UFCS accepts receiver types known before checker execution: function
  ...
  evaluation bridge, a method-argument hidden-receiver strip helper, a checked hidden-body package, a conditional hidden-body eval safety helper, and a behavior-preserving split
...
1. Finish UFCS receiver hardening.
   ...
   - Direct-call receivers are not an OCaml-only switch: existing store-safe
     ...
     hidden-body package for direct receiver-method summaries, a replay-parameterized hidden-let evaluation bridge, a method-argument hidden-receiver strip helper, a checked hidden-body package, a conditional hidden-body eval safety helper, and the active-core/public-base gate split
     now isolate direct and generic direct-call receiver method shapes without
     changing accepted programs. Next, remove the remaining conditional premises by proving callee-body
     replay under the added hidden receiver binding and by routing the hidden
