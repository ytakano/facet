...
- Short UFCS accepts receiver types known before checker execution: function
  ...
  lift constructors, plus variable-evaluation, place-level,
...
1. Finish UFCS receiver hardening.
   ...
   - Direct-call receivers are not an OCaml-only switch: existing store-safe
     ...
     The next proof step is proving the expression-level live/consumed
     reverse hidden-frame lift needed by the focused body bridge, then switching
