...
- Short UFCS accepts receiver types known before checker execution: function
  ...
  expressions, and struct/enum constructors. Public runtime branch wiring still
  needs that relation lifted
...
1. Finish UFCS receiver hardening.
   ...
   - Direct-call receivers are not an OCaml-only switch: existing store-safe
     ...
     assignment-wrapper, call-expression, direct-call, and aggregate-constructor
     helpers before using the hidden/base alpha transport
