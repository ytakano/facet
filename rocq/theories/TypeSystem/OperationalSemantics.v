...
Fixpoint copy_capture_store_as
    (captures : list ident) (params : list param) (s : store) : option store :=
...
Inductive eval (env : global_env) : store -> expr -> store -> value -> Prop :=
  ...
  | Eval_CallGeneric : forall s s_args s_body fname type_args fdef fcall args vs ret used',
      lookup_fn fname (env_fns env) = Some fdef ->
      fn_captures fdef = [] ->
      eval_args env s args s_args vs ->
      alpha_rename_fn_def (store_names s_args) fdef = (fcall, used') ->
