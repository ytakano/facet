...
From Facet.TypeSystem Require Export TypeSafetyPrefixPreservationLetRoots.
...
Proof.
  ...
  induction Htail as
    ...
    |R R_payload Rv_payload Rv Σ branches v rest e0 T
       ...
       Htyped Hno_lbound Hparams_ok Hroots_excl Hremove Henv_excl Hctx_remove
  ...
  - simpl in Hvariant.
    ...
    + eapply IHtail in Hvariant; [| exact Hbranch].
      ...
      destruct Hrest as [Hno_lbound' Hrest].
...
Lemma store_remove_params_store_app_prefix :
  forall ps entries frame,
    exists frame',
      store_remove_params ps (entries ++ frame) =
        store_remove_params ps entries ++ frame'.
Proof.
  induction ps as [| p ps IH]; intros entries frame.
  - exists frame. reflexivity.
  - simpl.
    destruct (store_remove_app_prefix (param_name p) entries frame)
      as [frame1 Hremove].
    rewrite Hremove.
    destruct (IH (store_remove (param_name p) entries) frame1)
      as [frame' Hremove_params].
    exists frame'. exact Hremove_params.
Qed.

Lemma store_typed_prefix_remove_params_excluding_final :
  forall env ps_store ps_ctx s Σ,
    ...
    store_typed_prefix env (store_remove_params ps_store s)
...
Proof.
  ...
  subst s.
  destruct (store_remove_params_store_app_prefix ps_store entries frame)
    as [frame' Hremove].
  unfold store_typed_prefix.
  exists (store_remove_params ps_store entries), frame'.
  split.
  - exact Hremove.
  - eapply store_typed_entries_remove_params_excluding_final
      with (ps_all := ps_store)
           (s_source := entries ++ frame).
    + exact Hnames.
    + reflexivity.
    + exact Htyped.
    + unfold store_no_shadow in Hshadow.
      rewrite store_names_app in Hshadow.
      eapply NoDup_app_remove_r. exact Hshadow.
    + exact Hexclude.
    + intros se Hin.
      rewrite Hremove.
...
Lemma store_remove_match_payload_cleanup_prefix_store_typed_names :
  forall ps_runtime ps_typed env s_body frame R roots v T Σ,
    ...
    store_typed_prefix env s_body Σ ->
    exists locals,
      ...
      store_typed_prefix env (store_remove_params ps_runtime s_body)
...
Proof.
  ...
  assert (Hstore_removed :
    store_typed_prefix env (store_remove_params ps_runtime s_body)
      (sctx_remove_params ps_typed Σ)).
  { eapply store_typed_prefix_remove_params_excluding_final; eassumption. }
  ...
  - intros s s1 s2 s3 x old_e e_new v_new Hlookup Heval_new
    ...
    + simpl in x0. inversion x0; subst.
      ...
      | Hready_new : provenance_ready_expr e_new,
        Hplace : typed_place_env_structural env Σ (PVar ?x_static) ?T_old,
        Htyped_new : typed_env_roots env Ω n R Σ e_new ?T_new ?Σ1 ?R1 ?roots_new,
        Hcompat : ty_compatible_b Ω ?T_new ?T_old = true,
        Havailable : sctx_path_available ?Σ1 ?x_static [] = infer_ok tt,
        Hrestore_ctx : sctx_restore_path ?Σ1 ?x_static [] = infer_ok Σ' |- _ =>
          destruct (typed_env_structural_preserves_var_target
                      env Ω n Σ Σ1 e_new T_new x_static T_old Hplace
          ...
          destruct (eval_replace_var_preserves_typing_prefix
                      env Ω n Σ Σ1 Σ' s s1 s2 s3 x_static old_e T_old T_new v_new
    ...
    + simpl in x0. inversion x0; subst.
      ...
      | Hready_new : provenance_ready_expr e_new,
        ...
        Hcompat : ty_compatible_b Ω ?T_new ?T_old = true |- _ =>
          destruct (typed_env_structural_preserves_var_target
                      env Ω n Σ Σ' e_new T_new x_static T_old Hplace
                      (typed_env_roots_structural env Ω n R Σ e_new
          ...
          destruct (eval_assign_var_preserves_typing_prefix
                      env Ω n Σ Σ' s s1 s2 x_static old_e T_old T_new v_new
                      Hstore Hplace Hstore1 Hvnew Hpres_new Hcompat
    ...
    + match goal with
      | Hready_new : provenance_ready_expr e_new,
        ...
        Hcompat : ty_compatible_b Ω ?T_new ?T_old = true |- _ =>
          ...
          rewrite Hlookup_runtime in Hlookup_old;
          rewrite Hvalue_runtime in Hlookup_old;
          inversion Hlookup_old; subst old_v; clear Hlookup_old;
          destruct (sctx_lookup_mut_some_lookup x_target Σ MMutable Hmut)
            as [T_root [st HΣ]];
          destruct (sctx_same_bindings_lookup Σ Σ1 x_target T_root st
                      (typed_env_structural_same_bindings env Ω n Σ e_new T_new Σ1
                        (typed_env_roots_structural env Ω n R Σ e_new
                          T_new Σ1 R1 roots_new Htyped_new)) HΣ)
            as [st1 HΣ1];
          destruct (store_typed_prefix_lookup_sctx
                      env s Σ x_target T_root st Hstore HΣ)
            as [se_static [Hlookup_static [_ [HTy [_ _]]]]];
          rewrite Hlookup_runtime in Hlookup_static;
          inversion Hlookup_static; subst se_static;
          destruct (type_lookup_path_lifetime_equiv env T_root (se_ty se)
                      path_eval T_eval (ty_lifetime_equiv_sym _ _ HTy)
                      Htype_runtime)
            as [T_update [Htype_update Hequiv_update]];
          assert (Hvnew_eval : value_has_type env s1 v_new T_eval)
          by (eapply VHT_LifetimeEquiv;
              [ eapply value_has_type_compatible;
                [ exact Hvnew
                | apply ty_compatible_b_sound with (Ω := Ω); exact Hcompat ]
              | apply ty_lifetime_equiv_sym; exact Hequiv_runtime ]);
          assert (Hvnew_update : value_has_type env s1 v_new T_update)
          by (eapply VHT_LifetimeEquiv;
              [ exact Hvnew_eval
              | apply ty_lifetime_equiv_sym; exact Hequiv_update ]);
          assert (Htarget_update : exists T_root0 st0,
                    sctx_lookup x_target Σ1 = Some (T_root0, st0) /\
                    type_lookup_path env T_root0 path_eval = Some T_update)
          by (exists T_root, st1; split; assumption);
          assert (Hpres_update : store_ref_targets_preserved env s1 s2)
          by (eapply store_update_path_ref_targets_preserved_prefix;
              [ exact Hstore1
              | exact Htarget_update
              | exact Hvnew_update
              | exact Hupdate ]);
          assert (Hstore2 : store_typed_prefix env s2 Σ1)
          by (eapply store_typed_prefix_update_path_typed;
              [ exact Hstore1
              | exact Hpres_update
              | exact Htarget_update
              | exact Hvnew_update
          ...
          by (unfold store_restore_path in Hrestore;
              eapply store_update_state_ref_targets_preserved; exact Hrestore);
          assert (Hstore_final : store_typed_prefix env s3 Σ1)
          by (eapply store_typed_prefix_restore_path_same_ctx; eassumption);
          assert (Hvold : value_has_type env s v_target T_old)
          by (eapply VHT_LifetimeEquiv; eassumption);
          assert (Hv_final : value_has_type env s3 v_target T_old)
          by (eapply value_has_type_store_preserved;
              [ exact Hvold
              | eapply store_ref_targets_preserved_trans;
                [ exact Hpres_new
                | eapply store_ref_targets_preserved_trans; eassumption ] ]);
    ...
    + match goal with
      | Hready_new : provenance_ready_expr e_new,
        ...
        Hcompat : ty_compatible_b Ω ?T_new ?T_old = true |- _ =>
          ...
          destruct (sctx_lookup_mut_some_lookup x_target Σ MMutable Hmut)
            as [T_root [st HΣ]];
          destruct (sctx_same_bindings_lookup Σ Σ' x_target T_root st
                      (typed_env_structural_same_bindings env Ω n Σ e_new T_new Σ'
                        (typed_env_roots_structural env Ω n R Σ e_new
                          T_new Σ' R1 roots_new Htyped_new)) HΣ)
            as [st1 HΣ1];
          destruct (store_typed_prefix_lookup_sctx
                      env s Σ x_target T_root st Hstore HΣ)
            as [se_static [Hlookup_static [_ [HTy [_ _]]]]];
          ...
          by (eapply VHT_LifetimeEquiv;
              ...
              | apply ty_lifetime_equiv_sym; exact Hequiv_update ]);
          assert (Htarget_update : exists T_root0 st0,
                    sctx_lookup x_target Σ' = Some (T_root0, st0) /\
                    type_lookup_path env T_root0 path_eval = Some T_update)
          by (exists T_root, st1; split; assumption);
          assert (Hpres_update : store_ref_targets_preserved env s1 s2)
          by (eapply store_update_path_ref_targets_preserved_prefix;
              ...
              | exact Hvnew_update
              | exact Hupdate ]);
          assert (Hstore_final : store_typed_prefix env s2 Σ')
          by (eapply store_typed_prefix_update_path_typed;
              [ exact Hstore1
              | exact Hpres_update
              | exact Htarget_update
  ...
  - intros s s_scrut s_branch s' scrut branches enum_name variant_name
    ...
    + apply String.eqb_eq in Hvariant_head. subst variant_name.
      assert (Hbranch_eq : e_branch = e_head).
      {
        eapply lookup_expr_branch_deterministic;
          [ exact Hlookup_branch | exact H13 ].
      }
      subst e_branch.
      ...
      { assert (Hedef_same : edef_runtime = edef) by congruence.
        ...
        match goal with
        | Hvariants : enum_variants edef = v_head :: v_tail |- _ =>
            rewrite Hvariants in Hlookup_variant_runtime
        end.
        simpl in Hlookup_variant_runtime.
      ...
      { rewrite Hlookup_binders in H8. inversion H8. reflexivity. }
      assert (Hparams_head :
        match_payload_params_opt binders_head lts args v_head =
          Some ps_head).
      { eapply match_payload_params_opt_infer_ok. exact H9. }
      ...
      pose proof (value_roots_within_enum_payloads roots_scrut enum_name0
        (enum_variant_name v_head) lts_val args_val values
        Hv_roots_scrut) as Hvalues_roots.
      ...
      destruct (store_remove_match_payload_cleanup_prefix_store_typed_names
        ...
        as [locals_branch [Hremoved_branch [Hv_removed
             [Hstore_refs_excl Hstore_removed]]]].
    ...
    + assert (Hvdef_runtime_tail : vdef_runtime = vdef_typed_runtime).
      ...
      destruct (typed_match_tail_roots_lookup env Ω n lts args R1
                  ...
                  variant_name vdef_runtime e_branch ltac:(eassumption)
      ...
      destruct Htail_branch as [Hno_lbound_branch Htail_branch].
      ...
      { rewrite (match_payload_params_opt_param_tys binders_branch lts args
        ...
        { match goal with
          ...
          simpl. rewrite Hvariant_head. exact Hvariant_runtime. }
        eapply (value_has_type_enum_values_lookup env s_scrut enum_name0
