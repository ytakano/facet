...
- Helper-level associated compatibility soundness is available for
  ...
  to store and prefix-store naming. Full-env, function end-to-end,
  function-list, program, and check-program entrypoints now expose both
  individual sctx/store naming facts and combined same-witness store packages
  for keys, roots, and returned root-set naming. Assoc root value and argument
  typing bridge to the same roots-ready invariants. Ordinary
