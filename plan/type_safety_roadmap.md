...
4. **S3: function call / current closure value safety** `[todo]`
   ...
   - `[todo]` direct `ECall` の pre-cleanup callee body preservation を、freshened body typing、freshened body readiness、`bind_params_store_typed_prefix`、parameter scope helper へ接続する。
   - `[todo]` `store_remove_params` 後の final preservation は、返り値と残存 store が removed parameter roots を参照しないことを root provenance / no-escape invariant で証明してから主張する。
