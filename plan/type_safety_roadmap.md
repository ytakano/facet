...
4. **S3: function call / current closure value safety** `[todo]`
   ...
   - `[partial]` `store_remove_params` 後の final preservation 用に、parameter name set 全体の root exclusion predicate / boolean soundness と `store_remove_params_cleanup_excludes` を追加済み。freshened callee body の prefix-compatible root preservation と parameter-scope evidence を cleanup helper に接続し、cleanup 後の return value typing と caller-visible `store_ref_targets_preserved` は証明済み。`store_frame_scope` evidence を明示前提にすると `store_typed env (store_remove_params ...) Σ_args` も復元できる。`store_frame_static_fresh` と、state/value/path update・mark/consume/restore・non-param remove が caller frame を exact に保存する `store_frame_scope_*` helper は追加済み。さらに `ELet` cleanup 用に、`store_no_shadow` 前提で runtime `store_remove x` と static `sctx_remove x` を同期する `store_frame_scope_remove_non_param_sctx_remove` / same-bindings bridge、および `sctx_lookup` / parameter freshness / absent lookup から frame freshness を導く補助補題を追加済み。`eval_preserves_frame_scope_roots_ready_args_fields_from_expr` で body evaluation の frame-scope preservation theorem の args/fields sequencing halves は切り出し済み。次は expression half を追加し、明示 frame premise を外す。
...
23. `[partial]` freshened callee body typing を `typed_env_structural` へ移す方針は global alpha-renaming forward theorem に固定済み。forward `ctx_alpha`/typed-place/args/fields/`expr_as_place` helper、`typed_env_structural` 本体の forward theorem、function-level bridge、`env_fns_preservation_ready`、direct-place `place_path` alpha-renaming helper、readiness alpha-renaming preservation、`preservation_direct_call_ready_expr` wrapper、lookup-to-freshened-callee ready/typed bridge、ready-prefix preservation theorem、direct `ECall` pre-cleanup body preservation bridge、root sidecar direct `ECall` checker/soundness、parameter-set cleanup exclusion helpers、prefix-compatible root preservation bridge、cleanup 後の return value / caller-visible reference preservation bridge、明示 `store_frame_scope` evidence からの cleanup exact typing bridge、`store_frame_static_fresh` と exact-frame store operation helper、`ELet` remove/context-remove 対応補題、`eval_preserves_frame_scope_roots_ready_args_fields_from_expr` による args/fields frame-scope sequencing は追加済み。次は `eval_preserves_frame_scope_roots_ready_mutual` の expression half を追加し、`store_remove_params` 後の final direct-call preservation theorem から明示 frame premise を外す。
