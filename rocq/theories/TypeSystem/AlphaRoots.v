...
Inductive typed_env_roots_shadow_safe
  ...
  | TERS_CallExpr_TypeForall : forall R R1 R' Σ Σ1 Σ' callee args u
      ...
      typed_env_roots_shadow_safe env Ω n R Σ (ECallExpr callee args)
        ...
        Σ' R' (root_set_union roots_callee (root_sets_union arg_roots))
  | TERS_CallExprGeneric_TypeForall : forall R R1 R' Σ Σ1 Σ' callee args u
        type_params bounds body_ty param_tys ret_inner type_args arg_roots roots_callee,
      typed_env_roots_shadow_safe env Ω n R Σ callee
        (MkTy u (TTypeForall type_params bounds body_ty)) Σ1 R1 roots_callee ->
      ty_core body_ty = TFn param_tys ret_inner ->
      ...
      typed_args_roots_shadow_safe env Ω n R1 Σ1 args
        (params_of_tys (map (subst_type_params_ty type_args) param_tys))
        Σ' R' arg_roots ->
      typed_env_roots_shadow_safe env Ω n R Σ (ECallExprGeneric callee type_args args)
        (subst_type_params_ty type_args ret_inner)
        Σ' R' (root_set_union roots_callee (root_sets_union arg_roots))
  ...
  | TERS_BorrowUnique_Resolved : forall R Σ p T roots x roots_x,
      ...
      typed_env_roots_shadow_safe env Ω n R Σ (EBorrow RUnique p)
        (MkTy UAffine (TRef (Lifetime.LVar n) RUnique T)) Σ R roots
  | TERS_BorrowUnique_ResolvedTarget : forall R Σ p T x,
      typed_place_env_structural env Σ p T ->
      place_path p = None ->
      place_under_unique_ref_structural env Σ p ->
      place_resolved_write_writable_chain env R Σ p ->
...
Lemma typed_env_roots_shadow_safe_call_expr_generic_typeforall_subst_type_params_bridge :
  forall env Omega n R R1 Sigma Sigma1 Sigma' R' callee args u type_params
      bounds body_ty param_tys ret_inner call_type_args outer_type_args
      arg_roots roots_callee,
    ...
    typed_args_roots_shadow_safe env Omega n R1 Sigma1 args
      (params_of_tys (map (subst_type_params_ty call_type_args) param_tys))
      Sigma' R' arg_roots ->
    typed_env_roots_shadow_safe env Omega n R
      (subst_type_params_ctx outer_type_args Sigma)
      (subst_type_params_expr outer_type_args callee)
      (MkTy u (TTypeForall type_params bounds body_ty))
      (subst_type_params_ctx outer_type_args Sigma1) R1 roots_callee ->
    (forall R0 Sigma0 e T0 Sigma0' R0' roots0,
        In e args ->
        typed_env_roots_shadow_safe env Omega n R0 Sigma0 e
          T0 Sigma0' R0' roots0 ->
    ...
    ty_lifetime_equiv
      (subst_type_params_ty outer_type_args
        (subst_type_params_ty call_type_args ret_inner))
      (subst_type_params_ty
        (map (subst_type_params_ty outer_type_args) call_type_args)
        ret_inner).
Proof.
  intros env Omega n R R1 Sigma Sigma1 Sigma' R' callee args u type_params
    bounds body_ty param_tys ret_inner call_type_args outer_type_args
    arg_roots roots_callee Hcallee Hbody Hbounds Hargs Hcallee_subst
    Hexpr_subst Hcompat_subst Hbounds_subst Hcompose.
  assert (Hargs_subst :
    typed_args_roots_shadow_safe env Omega n R1
      (subst_type_params_ctx outer_type_args Sigma1)
      (map (subst_type_params_expr outer_type_args) args)
      (params_of_tys
        (map (subst_type_params_ty
          (map (subst_type_params_ty outer_type_args) call_type_args))
          param_tys))
      (subst_type_params_ctx outer_type_args Sigma') R' arg_roots).
  {
    pose proof
      (typed_args_roots_shadow_safe_subst_type_params_expr_params_of_tys
        env Omega n R1 Sigma1 args
        (map (subst_type_params_ty call_type_args) param_tys)
        Sigma' R' arg_roots outer_type_args Hargs Hexpr_subst
        Hcompat_subst) as Hargs_subst.
    replace
      (params_of_tys
        (map (subst_type_params_ty
          (map (subst_type_params_ty outer_type_args) call_type_args))
      ...
      with
      (params_of_tys
        (map (subst_type_params_ty outer_type_args)
          (map (subst_type_params_ty call_type_args) param_tys))).
    - exact Hargs_subst.
    - rewrite map_map.
      apply f_equal. apply map_ext. intro T.
      rewrite subst_type_params_ty_compose.
      rewrite Hcompose. reflexivity.
  }
  split.
  - simpl.
...
Lemma typed_env_roots_shadow_safe_call_expr_generic_typeforall_subst_type_params_compat_package :
  forall env Omega n R Sigma callee call_type_args args T Sigma' R' roots
      outer_type_args,
    typed_env_roots_shadow_safe env Omega n R Sigma
      (ECallExprGeneric callee call_type_args args) T Sigma' R' roots ->
    ...
    exists T_subst Gamma_out_subst,
      ...
      ty_compatible_b Omega T_subst
        (subst_type_params_ty outer_type_args T) = true.
Proof.
  intros env Omega n R Sigma callee call_type_args args T Sigma' R' roots
    outer_type_args Htyped Hcallee_subst Hexpr_subst Hcompat_subst
    Hbounds_subst Hcompat_refl Hcompose.
  inversion Htyped; subst; try discriminate.
  match goal with
  | Hcallee : typed_env_roots_shadow_safe _ _ _ _ _ callee
      (MkTy ?u (TTypeForall ?type_params ?bounds ?body_ty)) _ _ ?roots_callee,
  ...
  destruct Hbridge as [Htyped_subst _].
  - eapply Hcallee_subst. eassumption.
  - exact Hexpr_subst.
  - exact Hcompat_subst.
  - apply Hbounds_subst. eassumption.
...
Proof.
  ...
  - intros R R1 Σ Σ1 p e_new T_old T_new roots_result x roots_old roots_new
    ...
    exists (root_env_update x (root_set_union roots_old0 roots_new0) R10),
      roots_result0.
    split; [| split; [| split]].
    + eapply TERS_Replace_Resolved; eauto; exact Hshape0.
    + apply root_env_no_shadow_update. exact HnsR10.
    + eapply root_env_equiv_trans with
        (R' := root_env_update x
          (root_set_union
  ...
  - intros R R1 Σ Σ' p e_new T_old T_new x roots_old roots_new
    ...
    exists (root_env_update x (root_set_union roots_old0 roots_new0) R10),
      [].
    split; [| split; [| split]].
    + eapply TERS_Assign_Resolved; eauto; exact Hshape0.
    + apply root_env_no_shadow_update. exact HnsR10.
    + eapply root_env_equiv_trans with
        (R' := root_env_update x
          (root_set_union
