...
- Runtime proof plumbing now has prefix-aware static-runtime callback shapes,
  ...
  match-tail branch lists, prefix-facing route wrappers through the mixed
...
- The concrete proof gap is deriving prefix/static-runtime evidence from the
  ...
  `store_typed_prefix`; and leaf/borrow argument, field, and match-tail branch
  lists have prefix traversals. The remaining static callback work is the mutual
  preservation-ready traversal for compound expressions, then packaging that
