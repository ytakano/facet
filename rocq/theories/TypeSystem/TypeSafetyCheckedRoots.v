...
Lemma sctx_remove_params_apply_type_params_checked_roots :
  forall type_args ps Sigma,
    sctx_remove_params (apply_type_params type_args ps) Sigma =
    sctx_remove_params ps Sigma.
Proof.
  intros type_args ps.
  induction ps as [| [m x T] ps IH]; intros Sigma; simpl.
  - reflexivity.
  - apply IH.
Qed.
...
Proof.
  ...
  - simpl.
    ...
    destruct (Hbranch v rest e T Σv_payload Rv_payload roots Σv
      Σs Ts rootss binders ps H H0 H1 H2 H3 H4 H6 H7 H8 H9 H12 H13 H14 Htail)
      as (ps_subst & Hparams_subst & Hnodup_subst & Hctx_none_subst &
        Hroot_none_subst & Htyped_subst & Hno_lbound_subst &
        Hparams_ok_subst & Hroots_exclude_subst & Hroot_excludes_subst &
        Hremove_subst &
...
Lemma typed_match_tail_roots_shadow_safe_apply_type_params_package :
  forall env Omega n lts enum_type_args R roots_scrut Sigma branches variants
      expected_core R_out Sigmas Ts roots_list type_args,
    compose_type_params type_args enum_type_args =
      map (subst_type_params_ty type_args) enum_type_args ->
    ...
    (forall v rest e T Sigma_v_payload Rv_payload roots Sigma_v
        ...
        root_env_excludes_params ps
          (root_env_remove_match_params ps Rv_payload) ->
        Sigma_v = sctx_remove_params ps Sigma_v_payload ->
        ty_core T = expected_core ->
        root_env_equiv (root_env_remove_match_params ps Rv_payload) R_out ->
        typed_match_tail_roots_shadow_safe env Omega n lts enum_type_args R
          ...
          Ts_tail roots_tail ->
        typed_env_roots_shadow_safe env Omega n
          (root_env_add_params_roots_same (apply_type_params type_args ps)
            roots_scrut R)
          ...
          Rv_payload roots /\
        contains_lbound_ty (subst_type_params_ty type_args T) = false /\
        params_ok_sctx_b env (apply_type_params type_args ps)
          (subst_type_params_ctx type_args Sigma_v_payload) = true /\
        ty_core (subst_type_params_ty type_args T) = expected_core) ->
    typed_match_tail_roots_shadow_safe env Omega n lts
      (map (subst_type_params_ty type_args) enum_type_args) R roots_scrut
...
Proof.
  intros env Omega n lts enum_type_args R roots_scrut Sigma branches variants
    expected_core R_out Sigmas Ts roots_list type_args Hcompose Htail
    Hbranch.
  induction Htail.
  - simpl. constructor.
  - simpl. subst R_payload Rv.
    destruct (Hbranch v rest e T Σv_payload Rv_payload roots Σv
      Σs Ts rootss binders ps H H0 H1 H2 H3 H4 H6 H7 H8 H9 H11 H12 H13 H14
      Htail) as [Htyped_subst [Hno_lbound_subst [Hparams_ok_subst Hcore_subst]]].
    eapply TERSMatchTail_Cons
      with (ps := apply_type_params type_args ps)
           (R_payload :=
              root_env_add_params_roots_same (apply_type_params type_args ps)
                roots_scrut R)
           (Σv_payload := subst_type_params_ctx type_args Σv_payload)
           (Rv_payload := Rv_payload)
           (Rv := root_env_remove_match_params
              (apply_type_params type_args ps) Rv_payload).
    + eapply lookup_expr_branch_binders_subst_type_params_match_branches.
      exact H.
    + eapply match_payload_params_subst_type_params_map; eassumption.
    + rewrite params_names_nodup_b_apply_type_params_checked_roots. exact H1.
    + rewrite ctx_lookup_params_none_b_subst_type_params_ctx. exact H2.
    + rewrite root_env_lookup_params_none_b_apply_type_params_checked_roots.
      exact H3.
    + eapply lookup_expr_branch_subst_type_params_match_branches. exact H4.
    + reflexivity.
    + exact Htyped_subst.
    + exact Hno_lbound_subst.
    + exact Hparams_ok_subst.
    + eapply roots_exclude_params_apply_type_params_checked_roots. exact H9.
    + reflexivity.
    + rewrite root_env_remove_match_params_apply_type_params_checked_roots.
      eapply root_env_excludes_params_apply_type_params_checked_roots.
      exact H11.
    + subst Σv.
      rewrite sctx_remove_params_apply_type_params_checked_roots.
      rewrite <- subst_type_params_ctx_sctx_remove_params.
      reflexivity.
    + exact Hcore_subst.
    + rewrite root_env_remove_match_params_apply_type_params_checked_roots.
      exact H14.
    + apply IHHtail.
      * exact Hcompose.
      * intros v0 rest0 e0 T0 Sigma_v_payload0 Rv_payload0 roots0 Sigma_v0
        Sigmas_tail0 Ts_tail0 roots_tail0 binders0 ps0 Hlookup_binders
        Hpayload Hnodup Hctx_none Hroot_none Hlookup_expr Htyped Hparams_ok
        Hroots_excl Hroot_excl Hremove Hcore Hroot_equiv Htail0.
      eapply Hbranch; eassumption.
Qed.

Lemma typed_env_roots_shadow_safe_ematch_subst_type_params_compat_package :
  forall env Omega n R Sigma scrut branches T Sigma' R' roots type_args,
    typed_env_roots_shadow_safe env Omega n R Sigma (EMatch scrut branches)
      T Sigma' R' roots ->
    (forall T_scrut Sigma1 R1 roots_scrut enum_name lts enum_type_args,
        typed_env_roots_shadow_safe env Omega n R Sigma scrut
          T_scrut Sigma1 R1 roots_scrut ->
        ty_core T_scrut = TEnum enum_name lts enum_type_args ->
        exists T_scrut_subst,
          typed_env_roots_shadow_safe env Omega n R
            (subst_type_params_ctx type_args Sigma)
            (subst_type_params_expr type_args scrut)
            T_scrut_subst (subst_type_params_ctx type_args Sigma1)
            R1 roots_scrut /\
          ty_core T_scrut_subst =
            TEnum enum_name lts
              (map (subst_type_params_ty type_args) enum_type_args)) ->
    (forall edef enum_type_args,
    ...
    (forall enum_type_args,
        compose_type_params type_args enum_type_args =
          map (subst_type_params_ty type_args) enum_type_args) ->
    (forall v rest e_branch T_branch Sigma_branch_payload R_branch_payload
        roots_branch Sigma_branch Sigmas_tail Ts_tail roots_tail binders ps
        lts enum_type_args R1 Sigma1 roots_scrut R_out T_head,
        lookup_expr_branch_binders (Program.enum_variant_name v) branches =
          Some binders ->
        match_payload_params binders lts enum_type_args v = infer_ok ps ->
        params_names_nodup_b ps = true ->
        ctx_lookup_params_none_b ps Sigma1 = true ->
        root_env_lookup_params_none_b ps R1 = true ->
        lookup_expr_branch (Program.enum_variant_name v) branches =
          Some e_branch ->
        typed_env_roots_shadow_safe env Omega n
          (root_env_add_params_roots_same ps roots_scrut R1)
          (sctx_add_params ps Sigma1) e_branch T_branch
          Sigma_branch_payload R_branch_payload roots_branch ->
        contains_lbound_ty T_branch = false ->
        params_ok_sctx_b env ps Sigma_branch_payload = true ->
        roots_exclude_params ps roots_branch ->
        root_env_excludes_params ps
          (root_env_remove_match_params ps R_branch_payload) ->
        Sigma_branch = sctx_remove_params ps Sigma_branch_payload ->
        ty_core T_branch = ty_core T_head ->
        root_env_equiv (root_env_remove_match_params ps R_branch_payload)
          R_out ->
        typed_match_tail_roots_shadow_safe env Omega n lts enum_type_args R1
          roots_scrut Sigma1 branches rest (ty_core T_head) R_out
          Sigmas_tail Ts_tail roots_tail ->
        typed_env_roots_shadow_safe env Omega n
          (root_env_add_params_roots_same (apply_type_params type_args ps)
            roots_scrut R1)
          (sctx_add_params (apply_type_params type_args ps)
            (subst_type_params_ctx type_args Sigma1))
          (subst_type_params_expr type_args e_branch)
          (subst_type_params_ty type_args T_branch)
          (subst_type_params_ctx type_args Sigma_branch_payload)
          R_branch_payload roots_branch /\
        contains_lbound_ty (subst_type_params_ty type_args T_branch) = false /\
        params_ok_sctx_b env (apply_type_params type_args ps)
          (subst_type_params_ctx type_args Sigma_branch_payload) = true /\
        ty_core (subst_type_params_ty type_args T_branch) = ty_core T_head) ->
    (forall Sigma_head Sigma_tail Gamma_out,
        ctx_merge_many (ctx_of_sctx Sigma_head)
          (map ctx_of_sctx Sigma_tail) = Some Gamma_out ->
        ctx_merge_many
          (ctx_of_sctx (subst_type_params_ctx type_args Sigma_head))
          (map ctx_of_sctx
            (map (subst_type_params_ctx type_args) Sigma_tail)) =
          Some (subst_type_params_ctx type_args Gamma_out)) ->
    (forall T_head Ts_tail T_head_subst,
        ty_core T_head_subst = ty_core T_head ->
        ty_compatible_b Omega
          (MkTy (usage_max_tys_nonempty T_head_subst
              (map (subst_type_params_ty type_args) Ts_tail))
            (ty_core T_head_subst))
...
Proof.
  intros env Omega n R Sigma scrut branches T Sigma' R' roots type_args
    Htyped Htransport_scrut Hbounds_subst Hcompose Hbranch Hmerge_subst
    Hcompat_result.
  inversion Htyped; subst.
  destruct (Htransport_scrut T_scrut Σ1 R1 roots_scrut enum_name lts args
    H1 H2) as [T_scrut_subst [Htyped_scrut_subst Hcore_scrut_subst]].
  destruct (Hbranch v_head v_tail e_head T_head Σ_head_payload
    R_head_payload roots_head (sctx_remove_params ps_head Σ_head_payload)
    Σ_tail Ts_tail roots_tail binders_head ps_head lts args R1 Σ1
    roots_scrut (root_env_remove_match_params ps_head R_head_payload)
    T_head H10 H11 H12 H13 H14 H15 H17 H18 H19 H20 H22 eq_refl
    eq_refl (root_env_equiv_refl _) H26)
    as [Htyped_head_subst [Hno_lbound_head_subst
      [Hparams_ok_head_subst Hcore_head_subst]]].
  pose proof
    (typed_match_tail_roots_shadow_safe_apply_type_params_package
      env Omega n lts args R1 roots_scrut Σ1 branches v_tail
      (ty_core T_head) (root_env_remove_match_params ps_head R_head_payload)
      Σ_tail Ts_tail roots_tail type_args (Hcompose args) H26)
    as Htail_package.
  pose proof (Hmerge_subst (sctx_remove_params ps_head Σ_head_payload)
    Σ_tail Γ_out H31) as Hmerge_subst_result.
  pose proof (Hcompat_result T_head Ts_tail
    (subst_type_params_ty type_args T_head) Hcore_head_subst)
    as Hcompat_match.
  exists (MkTy (usage_max_tys_nonempty (subst_type_params_ty type_args T_head)
      (map (subst_type_params_ty type_args) Ts_tail))
      (ty_core (subst_type_params_ty type_args T_head))),
    (subst_type_params_ctx type_args Γ_out).
  split.
  - simpl. rewrite subst_type_params_match_branches_go_map.
    change (sctx_of_ctx (subst_type_params_ctx type_args Γ_out)) with
      (subst_type_params_ctx type_args (sctx_of_ctx Γ_out)).
    eapply TERS_Match.
    + exact Htyped_scrut_subst.
    + exact Hcore_scrut_subst.
    + eassumption.
    + eassumption.
    + rewrite length_map. eassumption.
    + eapply Hbounds_subst; eassumption.
    + exact H7.
    + eapply first_unknown_variant_branch_subst_type_params_match_branches.
      eassumption.
    + eassumption.
    + eapply lookup_expr_branch_binders_subst_type_params_match_branches.
      eassumption.
    + eapply match_payload_params_subst_type_params_map; eauto.
    + rewrite params_names_nodup_b_apply_type_params_checked_roots. eassumption.
    + rewrite ctx_lookup_params_none_b_subst_type_params_ctx. eassumption.
    + rewrite root_env_lookup_params_none_b_apply_type_params_checked_roots.
      eassumption.
    + eapply lookup_expr_branch_subst_type_params_match_branches.
      eassumption.
    + reflexivity.
    + exact Htyped_head_subst.
    + exact Hno_lbound_head_subst.
    + exact Hparams_ok_head_subst.
    + eapply roots_exclude_params_apply_type_params_checked_roots.
      eassumption.
    + rewrite root_env_remove_match_params_apply_type_params_checked_roots.
      reflexivity.
    + eapply root_env_excludes_params_apply_type_params_checked_roots.
    ...
    + rewrite sctx_remove_params_apply_type_params_checked_roots.
      rewrite <- subst_type_params_ctx_sctx_remove_params.
      reflexivity.
    + rewrite Hcore_head_subst. apply Htail_package.
      intros v rest e T0 Sigma_v_payload Rv_payload roots0 Sigma_v
        Sigmas_tail0 Ts_tail0 roots_tail0 binders ps Hlookup_binders Hpayload
        Hnodup Hctx_none Hroot_none Hlookup_expr Htyped_branch Hno_lbound
        Hparams_ok Hroots_excl Hroot_excl Hremove Hcore Hroot_equiv Htail0.
      eapply Hbranch; eassumption.
    + exact Hmerge_subst_result.
  - exact Hcompat_match.
Qed.
...
Lemma typed_env_roots_shadow_safe_enum_provenance_ready_leaf_subst_type_params_package :
  forall env Omega n R Sigma enum_name variant_name lts variant_lts enum_type_args
    ...
    typed_args_roots_shadow_safe env Omega n R Sigma payloads
      (params_of_tys
        (map (instantiate_enum_variant_field_ty lts variant_lts enum_type_args)
    ...
    exists T_subst Gamma_out_subst,
      typed_env_roots_shadow_safe env Omega n R
        (subst_type_params_ctx type_args Sigma)
        (subst_type_params_expr type_args
          (EEnum enum_name variant_name lts variant_lts enum_type_args payloads))
      ...
      ty_compatible_b Omega T_subst
        (subst_type_params_ty type_args
          (instantiate_enum_ty edef lts enum_type_args)) = true.
Proof.
  intros env Omega n R Sigma enum_name variant_name lts variant_lts enum_type_args
    payloads edef vdef Sigma' R' payload_roots type_args Hready Hlookup
    Hlookup_variant Hlen_lts Hlen_variant_lts Hlen_args _ Houtlives
    Hbounds_subst Htyped_payloads
  ...
  - simpl. rewrite subst_type_params_expr_list_go_map.
    ...
    + assert (Hpayload_tys :
        ...
        map (instantiate_enum_variant_field_ty lts variant_lts
          (map (subst_type_params_ty type_args) enum_type_args))
          (Program.enum_variant_fields vdef)).
      { rewrite <- Hcompose.
        rewrite map_map.
...
Lemma typed_env_roots_shadow_safe_struct_provenance_ready_subst_type_params_package :
  forall env Omega n R Sigma sname lts struct_type_args fields
      sdef Sigma' R' roots type_args,
    provenance_ready_fields fields ->
    Program.lookup_struct sname env = Some sdef ->
...
Proof.
  intros env Omega n R Sigma sname lts struct_type_args fields
    sdef Sigma' R' roots type_args Hready Hlookup Hlen_lts Hlen_args
    _ Hbounds_subst Htyped_fields Htransport Hcompat_transport
    Hcompat_result Hcompose.
  exists (instantiate_struct_instance_ty sdef lts
      (map (subst_type_params_ty type_args) struct_type_args)),
    (subst_type_params_ctx type_args Sigma').
  split.
  - simpl. rewrite subst_type_params_fields_go_map.
    eapply TERS_Struct; eauto.
    + rewrite length_map. exact Hlen_args.
    + change (sctx_of_ctx (subst_type_params_ctx type_args Sigma')) with
        (subst_type_params_ctx type_args Sigma').
      eapply typed_fields_roots_shadow_safe_provenance_ready_subst_type_params_package;
        eauto.
  - exact Hcompat_result.
Qed.

Lemma typed_env_roots_shadow_safe_enum_provenance_ready_subst_type_params_package :
  forall env Omega n R Sigma enum_name variant_name lts variant_lts enum_type_args
      payloads edef vdef Sigma' R' payload_roots type_args,
    provenance_ready_args payloads ->
    Program.lookup_enum enum_name env = Some edef ->
...
Proof.
  intros env Omega n R Sigma enum_name variant_name lts variant_lts enum_type_args
    ...
    Hlookup_variant Hlen_lts Hlen_variant_lts Hlen_args _ Houtlives
    Hbounds_subst Htyped_payloads
    Htransport Hcompat_transport Hcompat_result Hcompose.
  exists (instantiate_enum_ty edef lts
      (map (subst_type_params_ty type_args) enum_type_args)),
    (subst_type_params_ctx type_args Sigma').
  split.
  - simpl. rewrite subst_type_params_expr_list_go_map.
    ...
    + assert (Hpayload_tys :
      ...
      change (sctx_of_ctx (subst_type_params_ctx type_args Sigma')) with
        (subst_type_params_ctx type_args Sigma').
      eapply typed_args_roots_shadow_safe_provenance_ready_subst_type_params_package;
        eauto.
  - exact Hcompat_result.
Qed.
  ...
  forall env Omega n R Sigma e T Sigma' R' roots type_args,
    ...
    (forall sname lts struct_type_args fields T0 Sigma0' R0' roots0,
        ...
        exists T_subst Gamma_out_subst,
          ...
          ty_compatible_b Omega T_subst
            (subst_type_params_ty type_args T0) = true) ->
    (forall enum_name variant_name lts variant_lts enum_type_args payloads
        T0 Sigma0' R0' roots0,
        provenance_ready_args payloads ->
        typed_env_roots_shadow_safe env Omega n R Sigma
          (EEnum enum_name variant_name lts variant_lts enum_type_args payloads)
