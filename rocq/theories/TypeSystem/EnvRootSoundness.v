          ...
          | _ = TForall ?m ?bounds ?body =>
            ...
            | _ = TFn ?param_tys ?ret =>
              ...
              destruct (check_arg_tys Ω arg_tys (map (open_bound_ty σ) param_tys))
                as [err |] eqn:Hcheck;
                cbn iota in Hinfer; try discriminate;
              destruct (contains_lbound_ty (open_bound_ty σ ret) ||
                        contains_lbound_outlives (open_bound_outlives σ bounds))
                eqn:Hlbound;
                cbn iota in Hinfer; try discriminate;
              destruct (outlives_constraints_hold_b Ω (open_bound_outlives σ bounds))
    ...
    + destruct (infer_place_sctx env Σ p) as [Tp | err] eqn:Hplace; try discriminate.
      ...
      * destruct r.
        ...
        -- destruct (place_under_unique_ref_b env Σ p) eqn:Hunique; try discriminate.
           destruct (place_resolved_write_writable_chain_b env R Σ p) eqn:Hchain.
           { destruct (place_resolved_write_target R p) as [target_x |] eqn:Htarget.
             { change (infer_ok (MkTy UAffine (TRef (LVar n) RUnique Tp), Σ, R, [RStore target_x]) =
                 infer_ok (T, Σ', R', roots)) in Hinfer.
               injection Hinfer as Hroots_eq HR_eq HΣ_eq HT_eq.
               rewrite <- HT_eq. rewrite <- HΣ_eq. rewrite <- HR_eq. rewrite <- Hroots_eq.
               exact (@TER_BorrowUnique_ResolvedTarget env Ω n R Σ p Tp target_x
                 (infer_place_sctx_structural_sound env Σ p Tp Hplace)
                 Hpath
                 (place_under_unique_ref_b_sound env Σ p Hunique)
  ...
  - simpl in Hinfer. discriminate.
    ...
    + destruct (infer_core_env_state_fuel_roots_shadow_safe fuel' env Ω n R Σ e)
      ...
      remember
        ((fix infer_rest (T_head0 : Ty) (R_out0 : root_env)
            ...
            end) T_head (root_env_remove_match_params ps_head R_head_payload) v_tail)
        as tail_result eqn:Htail.
      destruct tail_result as [[[Σ_tail Ts_tail] roots_tail] | err_tail];
        try discriminate.
      symmetry in Htail.
      assert (Htail_typed :
      ...
      destruct (ctx_merge_many
          ...
          (map ctx_of_sctx Σ_tail)) as [Γ_out |] eqn:Hmerge; try discriminate.
      assert (Hcalc :
        infer_core_env_state_fuel_roots_shadow_safe (S fuel') env Ω n R Σ (EMatch e l) =
        infer_ok
          (MkTy (usage_max_tys_nonempty T_head Ts_tail) (ty_core T_head),
           sctx_of_ctx Γ_out,
           root_env_remove_match_params ps_head R_head_payload,
    ...
    + destruct (infer_place_sctx env Σ p) as [Tp | err] eqn:Hplace; try discriminate.
      ...
      * destruct r.
        -- destruct (place_resolved_roots R p) as [roots_res |] eqn:Hresolved.
           ++ destruct (singleton_store_root roots_res) as [root_x |] eqn:Hsingle.
              ** destruct (root_env_lookup root_x R) as [roots_x |] eqn:Hlookup_x.
                 { destruct (singleton_store_root roots_x) as [root_y |] eqn:Hsingle_x.
        ...
        -- destruct (place_under_unique_ref_b env Σ p) eqn:Hunique; try discriminate.
           ...
           { destruct (place_resolved_write_target R p) as [target_x |] eqn:Htarget.
             { change (infer_ok (MkTy UAffine (TRef (LVar n) RUnique Tp), Σ, R, [RStore target_x]) =
               ...
               injection Hinfer as Hroots_eq HR_eq HΣ_eq HT_eq.
               rewrite <- HT_eq. rewrite <- HΣ_eq. rewrite <- HR_eq. rewrite <- Hroots_eq.
               exact (@TERS_BorrowUnique_ResolvedTarget env Ω n R Σ p Tp target_x
                 (infer_place_sctx_structural_sound env Σ p Tp Hplace)
                 Hpath
                 (place_under_unique_ref_b_sound env Σ p Hunique)
                 (place_resolved_write_writable_chain_b_sound env R Σ p Hchain)
