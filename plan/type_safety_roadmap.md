...
Last updated implementation point: added the first static borrow-state safety invariant. `BorrowStateSafety.v` defines `pbs_no_conflicts` for active path borrows, proves structural borrow checking preserves it, and connects the invariant to `borrow_check_env` and root-sidecar checked functions. Runtime aliasing correspondence is still future work.
...
- S5: `[partial]` ready/root-provenance fragment で runtime refs の no-dangling theorem、direct ref membership target theorem、static borrow-state no-conflict invariant は追加済み。runtime aliasing correspondence と captured closure refs は未着手。
...
19. `[partial]` `RuntimeRefSafety.v` に ready/root-provenance fragment の no-dangling runtime reference theorem 群と direct ref membership target theorem 群を追加し、`BorrowStateSafety.v` に static borrow-state no-conflict invariant を追加する。runtime aliasing correspondence と captured closure refs は未完了。
