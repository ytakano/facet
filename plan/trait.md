...
- Short UFCS accepts receiver types known before checker execution: function
  ...
  inferred let expression bodies, deref/drop unary wrappers, and replace/assign
  wrappers. Public runtime branch wiring still needs that relation lifted
...
1. Finish UFCS receiver hardening.
   ...
   - Direct-call receivers are not an OCaml-only switch: existing store-safe
     ...
     let-binder seed, conditional-chain, let-body, unary-wrapper, and
     assignment-wrapper helpers before using the hidden/base alpha transport
