...
4. **S3: function call / current closure value safety** `[todo]`
   ...
   - `[partial]` `store_remove_params` 後の final preservation 用に、parameter name set 全体の root exclusion predicate / boolean soundness と `store_remove_params_cleanup_excludes` を追加済み。次は freshened callee body の root evidence と parameter-scope evidence をこの cleanup helper に接続して final theorem を主張する。
