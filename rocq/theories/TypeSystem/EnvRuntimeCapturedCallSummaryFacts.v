...
Definition component_body_no_capture_direct_call_component_alpha_nested_target_lookup_in_provider
    (env : global_env) : Prop :=
  forall f_component,
    In f_component (env_fns env) ->
    callee_body_root_shadow_no_capture_direct_call_component_store_safe_summary
      env f_component ->
    forall fname_component args_component synthetic_component,
      direct_call_target_expr (fn_body f_component) =
        Some (fname_component, args_component, synthetic_component) ->
      forall fdef,
        lookup_fn fname_component
          (env_fns (global_env_with_local_bounds env (fn_bounds f_component))) =
          Some fdef ->
        callee_body_root_shadow_no_capture_direct_call_component_store_safe_summary
          (global_env_with_local_bounds env (fn_bounds f_component)) fdef ->
        forall fcall used used' fname args synthetic_body,
...
Definition component_body_synthetic_direct_call_ready_alpha_nested_body_env_evidence_in_provider
    (env : global_env) : Prop :=
  forall f_component,
    In f_component (env_fns env) ->
    callee_body_root_shadow_no_capture_direct_call_component_store_safe_summary
...
Proof.
  intros env fdef Hsummary.
  unfold callee_body_root_shadow_captured_call_generic_direct_narrow_store_safe_summary in Hsummary.
  unfold check_fn_root_shadow_generic_direct_store_safe_summary in Hsummary.
  ...
  pose proof
    (check_fn_root_shadow_generic_direct_store_safe_summary_target_view_sound
      env fdef fname type_args args synthetic_body Hsummary) as Htarget_view.
  destruct Htarget_view as
    [fcallee T_body Gamma_body R_body roots_body Hsafe_args Hin_callee
      Hname_callee Htype_params Hbounds Hbody_env Hcallee_ready Hcallee_expr
      Hroots Henv].
  ...
  eapply CBRSSNI_GenericDirect with
    (raw_body := fn_body fdef) (synthetic_body := synthetic_body)
    (fcallee := fcallee) (T_body := T_body_core) (Gamma_out := Gamma_body)
    (R_body := R_body) (roots_body := roots_body).
  - rewrite subst_type_params_expr_nil. reflexivity.
  - exact Htarget.
  ...
  - rewrite subst_type_params_trait_bounds_nil.
    ...
    exact Hbody_core.
  - rewrite subst_type_params_ty_nil. exact Hbody_compat.
...
Lemma generic_direct_call_target_expr_synthetic :
  forall body fname type_args args synthetic_body,
    generic_direct_call_target_expr body =
      Some (fname, type_args, args, synthetic_body) ->
    synthetic_body = ECallGeneric fname type_args args.
Proof.
  intros body fname type_args args synthetic_body Htarget.
  unfold generic_direct_call_target_expr in Htarget.
  destruct body; try discriminate.
  inversion Htarget. reflexivity.
Qed.


Lemma check_fn_root_shadow_direct_receiver_method_store_safe_summary_view_prop :
  forall env fdef,
    ...
    exists method_name type_args receiver_name receiver_args method_args
      ...
      method_callee T_receiver Gamma_receiver R_receiver roots_receiver
      T_receiver_env Gamma_receiver_env R_receiver_env roots_receiver_env
      ret_roots_receiver T_method Gamma_method R_method roots_method
      T_method_env Gamma_method_env R_method_env roots_method_env
      ...
      infer_core_env_roots_shadow_safe env
        (fn_outlives receiver_callee) (fn_lifetimes receiver_callee)
        (initial_root_env_for_fn receiver_callee)
        (fn_body_ctx receiver_callee) (fn_body receiver_callee) =
        infer_ok (T_receiver, Gamma_receiver, R_receiver, roots_receiver) /\
      infer_env_roots_shadow_safe env receiver_callee
        (initial_root_env_for_fn receiver_callee) =
        infer_ok (T_receiver_env, Gamma_receiver_env, R_receiver_env,
          roots_receiver_env) /\
      expr_root_shadow_store_safe_narrow_summary env
        (fn_outlives receiver_callee) (fn_lifetimes receiver_callee)
        (initial_root_env_for_fn receiver_callee)
        (sctx_of_ctx (fn_body_ctx receiver_callee))
        (fn_body receiver_callee) T_receiver (sctx_of_ctx Gamma_receiver)
        R_receiver roots_receiver ret_roots_receiver /\
      ty_compatible_b (fn_outlives receiver_callee) T_receiver
        (fn_ret receiver_callee) = true /\
      roots_exclude_params (fn_params receiver_callee) roots_receiver /\
      root_env_excludes_params (fn_params receiver_callee) R_receiver /\
      infer_core_env_roots_shadow_safe
        (global_env_with_local_bounds env
          (subst_type_params_trait_bounds type_args
            (fn_bounds method_callee)))
        (fn_outlives method_callee) (fn_lifetimes method_callee)
        (initial_root_env_for_fn method_callee)
        (subst_type_params_ctx type_args (fn_body_ctx method_callee))
        (subst_type_params_expr type_args (fn_body method_callee)) =
        infer_ok (T_method, Gamma_method, R_method, roots_method) /\
      infer_env_roots_shadow_safe env method_callee
        (initial_root_env_for_fn method_callee) =
        infer_ok (T_method_env, Gamma_method_env, R_method_env,
          roots_method_env) /\
      ...
      ty_compatible_b (fn_outlives method_callee) T_method
        (subst_type_params_ty type_args (fn_ret method_callee)) = true /\
      roots_exclude_params
...
Proof.
  ...
  destruct (lookup_fn_b receiver_name (env_fns env))
    as [receiver_callee |] eqn:Hlookup_receiver; try discriminate.
  destruct (lookup_fn_b method_name (env_fns env))
    as [method_callee |] eqn:Hlookup_method; try discriminate.
  apply andb_true_iff in Hcheck as [Htype_params Hcheck].
  apply Nat.eqb_eq in Htype_params.
  destruct (check_struct_bounds
    (global_env_with_local_bounds env (fn_bounds fdef))
    (fn_bounds method_callee) type_args) as [bounds_err |]
    eqn:Hbounds; try discriminate.
  destruct (infer_core_env_roots_shadow_safe env
    (fn_outlives receiver_callee) (fn_lifetimes receiver_callee)
    (initial_root_env_for_fn receiver_callee) (fn_body_ctx receiver_callee)
    (fn_body receiver_callee))
    as [[[[T_receiver Gamma_receiver] R_receiver] roots_receiver] | err]
    eqn:Hreceiver_core; try discriminate.
  destruct (infer_env_roots_shadow_safe env receiver_callee
    (initial_root_env_for_fn receiver_callee))
  ...
  destruct (infer_env_roots_shadow_safe env method_callee
    (initial_root_env_for_fn method_callee))
    as [[[[T_method_env Gamma_method_env] R_method_env]
          roots_method_env] | err] eqn:Hmethod_env; try discriminate.
  remember (direct_call_receiver_method_hidden_let_synthetic_body
    (fn_ret receiver_callee) method_name type_args receiver_name
    receiver_args method_args) as hidden_synthetic_body eqn:Hhidden.
  destruct (infer_env_roots_shadow_safe env
    (fn_with_body fdef hidden_synthetic_body)
    (initial_root_env_for_fn fdef))
    as [[[[T_body Gamma_body] R_out] roots] | err]
    eqn:Hbody_env; try discriminate.
  repeat rewrite andb_true_iff in Hcheck.
  destruct Hcheck as
    [[[[[[[[[[[Hreceiver_summary Hreceiver_compat] Hreceiver_roots]
              Hreceiver_env_excl] Hmethod_ready] Hmethod_summary]
            Hmethod_compat] Hmethod_roots] Hmethod_env_excl]
          Hbody_compat] Hroots] Henv].
  ...
  destruct (check_expr_root_shadow_store_safe_narrow_summary_sound
    env (fn_outlives receiver_callee) (fn_lifetimes receiver_callee)
    (initial_root_env_for_fn receiver_callee) (fn_body_ctx receiver_callee)
    (fn_body receiver_callee) T_receiver Gamma_receiver R_receiver
    roots_receiver Hreceiver_core Hreceiver_summary)
    as [ret_roots_receiver Hreceiver_summary_prop].
  pose proof
    (check_callee_body_root_shadow_store_safe_narrow_summary_instantiated_sound
      env method_callee type_args Hmethod_summary) as Hmethod_summary_prop.
  exists method_name, type_args, receiver_name, receiver_args, method_args,
    target_synthetic_body, hidden_synthetic_body, receiver_callee,
    method_callee, T_receiver, Gamma_receiver, R_receiver, roots_receiver,
    T_receiver_env, Gamma_receiver_env, R_receiver_env, roots_receiver_env,
...
Lemma check_fn_root_shadow_generic_direct_receiver_method_store_safe_summary_view_prop :
  forall env fdef,
    ...
    exists method_name type_args receiver_name receiver_type_args receiver_args
      ...
      method_callee T_receiver_env Gamma_receiver_env R_receiver_env
      roots_receiver_env T_method_env Gamma_method_env R_method_env
      roots_method_env T_body Gamma_body R_out roots,
...
Proof.
  ...
  destruct (lookup_fn_b method_name (env_fns env))
    as [method_callee |] eqn:Hlookup_method; try discriminate.
  apply andb_true_iff in Hcheck as [Htype_params Hcheck].
  apply andb_true_iff in Htype_params as
  ...
  destruct (check_struct_bounds
    (global_env_with_local_bounds env (fn_bounds fdef))
    (fn_bounds receiver_callee) receiver_type_args) as [bounds_receiver |]
    eqn:Hbounds_receiver; try discriminate.
  destruct (check_struct_bounds
    (global_env_with_local_bounds env (fn_bounds fdef))
    (fn_bounds method_callee) type_args) as [bounds_method |]
    eqn:Hbounds_method; try discriminate.
  destruct (infer_env_roots_shadow_safe env receiver_callee
    (initial_root_env_for_fn receiver_callee))
    as [[[[T_receiver_env Gamma_receiver_env] R_receiver_env]
          roots_receiver_env] | err] eqn:Hreceiver_env; try discriminate.
  destruct (infer_env_roots_shadow_safe env method_callee
    ...
    as [[[[T_method_env Gamma_method_env] R_method_env]
          roots_method_env] | err] eqn:Hmethod_env; try discriminate.
  remember (generic_direct_call_receiver_method_hidden_let_synthetic_body
    (subst_type_params_ty receiver_type_args (fn_ret receiver_callee))
    method_name type_args receiver_name receiver_type_args receiver_args
    method_args) as hidden_synthetic_body eqn:Hhidden.
  destruct (infer_env_roots_shadow_safe env
    (fn_with_body fdef hidden_synthetic_body)
    (initial_root_env_for_fn fdef))
    as [[[[T_body Gamma_body] R_out] roots] | err]
  ...
  repeat rewrite andb_true_iff in Hcheck.
  destruct Hcheck as
    [[[[[[Hreceiver_ready Hreceiver_summary] Hmethod_ready]
        Hmethod_summary] Hbody_compat] Hroots] Henv].
  apply lookup_fn_b_sound in Hlookup_receiver.
  destruct Hlookup_receiver as [Hin_receiver Hname_receiver].
  apply lookup_fn_b_sound in Hlookup_method.
  destruct Hlookup_method as [Hin_method Hname_method].
  pose proof
    (check_callee_body_root_shadow_store_safe_narrow_summary_instantiated_sound
      env receiver_callee receiver_type_args Hreceiver_summary)
  ...
  exists method_name, type_args, receiver_name, receiver_type_args,
    ...
    receiver_callee, method_callee, T_receiver_env, Gamma_receiver_env,
    R_receiver_env, roots_receiver_env, T_method_env, Gamma_method_env,
...
Definition check_fn_root_shadow_generic_direct_store_safe_summary_sound :
  forall env fdef,
    check_fn_root_shadow_generic_direct_store_safe_summary env fdef = true ->
    callee_body_root_shadow_captured_call_generic_direct_narrow_store_safe_summary
      env fdef :=
  fun env fdef Hgeneric => Hgeneric.


Definition check_fn_root_shadow_direct_receiver_method_store_safe_summary_sound :
  forall env fdef,
    check_fn_root_shadow_direct_receiver_method_store_safe_summary env fdef = true ->
    callee_body_root_shadow_captured_call_direct_receiver_method_narrow_store_safe_summary
      env fdef :=
  fun env fdef Hcheck => Hcheck.


Definition check_fn_root_shadow_generic_direct_receiver_method_store_safe_summary_sound :
  forall env fdef,
    check_fn_root_shadow_generic_direct_receiver_method_store_safe_summary env fdef = true ->
...
Proof.
  ...
  unfold callee_body_root_shadow_captured_call_store_safe_summary.
  eapply check_fn_root_shadow_captured_call_base_store_safe_summary_sound.
  exact Hcheck.
Qed.


Definition callee_body_root_shadow_captured_call_store_safe_summary_with_direct_receiver_method
    (env : global_env) (fdef : fn_def) : Prop :=
  callee_body_root_shadow_captured_call_store_safe_summary env fdef \/
  callee_body_root_shadow_captured_call_direct_receiver_method_narrow_store_safe_summary
    env fdef.

Lemma check_fn_root_shadow_captured_call_store_safe_summary_with_direct_receiver_method_sound :
  forall env fdef,
...
Proof.
  ...
  unfold check_fn_root_shadow_captured_call_store_safe_summary_with_direct_receiver_method
    in Hcheck.
  unfold callee_body_root_shadow_captured_call_store_safe_summary_with_direct_receiver_method.
...
Definition env_fns_root_shadow_captured_call_store_safe_summary_with_direct_receiver_method_check_ready
    (env : global_env) : Prop :=
  forall fname fdef,
    lookup_fn fname (env_fns env) = Some fdef ->
    callee_body_root_shadow_captured_call_store_safe_summary_with_direct_receiver_method
...
Lemma check_env_root_shadow_captured_call_store_safe_summary_with_direct_receiver_method_ready :
  forall env,
    check_env_root_shadow_captured_call_store_safe_summary_with_direct_receiver_method
      env = true ->
    env_fns_root_shadow_captured_call_store_safe_summary_with_direct_receiver_method_check_ready
      env.
Proof.
  intros env Hcheck fname fdef Hlookup.
  unfold check_env_root_shadow_captured_call_store_safe_summary_with_direct_receiver_method
    in Hcheck.
...
Definition callee_body_root_shadow_direct_receiver_method_or_no_capture_direct_component_store_safe_summary
    (env : global_env) (fdef : fn_def) : Prop :=
  callee_body_root_shadow_captured_call_direct_receiver_method_narrow_store_safe_summary
    env fdef \/
  callee_body_root_shadow_no_capture_direct_call_component_store_safe_summary
...
Definition env_fns_root_shadow_direct_receiver_method_or_no_capture_direct_component_store_safe_summary_ready
    (env : global_env) : Prop :=
  forall fname fdef,
    lookup_fn fname (env_fns env) = Some fdef ->
...
Lemma check_fn_root_shadow_direct_receiver_method_or_no_capture_direct_component_store_safe_summary_sound :
  forall env fdef,
...
Proof.
  intros env fdef Hcheck.
  unfold check_fn_root_shadow_direct_receiver_method_or_no_capture_direct_component_store_safe_summary
    in Hcheck.
  unfold callee_body_root_shadow_direct_receiver_method_or_no_capture_direct_component_store_safe_summary.
  apply orb_true_iff in Hcheck as [Hdirect | Hcomponent].
  - left.
    apply check_fn_root_shadow_direct_receiver_method_store_safe_summary_sound.
    exact Hdirect.
  - right.
    apply check_fn_root_shadow_no_capture_direct_call_component_store_safe_summary_sound.
    exact Hcomponent.
...
Lemma check_env_root_shadow_direct_receiver_method_or_no_capture_direct_component_store_safe_summary_ready :
  forall env,
...
Definition callee_body_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary
    (env : global_env) (fdef : fn_def) : Prop :=
  callee_body_root_shadow_captured_call_store_safe_summary env fdef \/
  (check_fn_root_shadow_no_capture_direct_call_component_store_safe_summary
      env fdef = true /\
...
Definition env_fns_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary_ready
    (env : global_env) : Prop :=
  forall fname fdef,
    lookup_fn fname (env_fns env) = Some fdef ->
...
Definition callee_body_root_shadow_captured_call_store_safe_with_direct_receiver_method_or_no_capture_direct_component_summary
    (env : global_env) (fdef : fn_def) : Prop :=
  callee_body_root_shadow_captured_call_store_safe_summary_with_direct_receiver_method
    env fdef \/
  callee_body_root_shadow_no_capture_direct_call_component_store_safe_summary
...
Definition env_fns_root_shadow_captured_call_store_safe_with_direct_receiver_method_or_no_capture_direct_component_summary_ready
    (env : global_env) : Prop :=
  forall fname fdef,
    lookup_fn fname (env_fns env) = Some fdef ->
...
Lemma check_fn_root_shadow_captured_call_store_safe_with_direct_receiver_method_or_no_capture_direct_component_summary_sound :
  forall env fdef,
...
Lemma check_env_root_shadow_captured_call_store_safe_with_direct_receiver_method_or_no_capture_direct_component_summary_ready :
  forall env,
...
Lemma component_body_no_capture_direct_call_component_target_in_of_closure_check :
  forall env f_component fname args synthetic_body fdef,
    fn_env_unique_by_name env ->
    In f_component (env_fns env) ->
    callee_body_root_shadow_no_capture_direct_call_component_store_safe_summary
      env f_component ->
    ...
    callee_body_root_shadow_no_capture_direct_call_component_store_safe_summary
      (global_env_with_local_bounds env (fn_bounds f_component)) fdef.
Proof.
  intros env f_component fname args synthetic_body fdef Hunique Hin_component
    Hcomponent Hcheck Htarget Hlookup.
  change (lookup_fn fname (env_fns env) = Some fdef) in Hlookup.
  pose proof (lookup_fn_b_of_lookup_fn fname (env_fns env) fdef Hlookup)
    as Hlookup_b.
  unfold check_fn_root_shadow_no_capture_direct_call_component_closure
...
Lemma component_body_no_capture_direct_call_component_target_in_of_exact_closure_check :
  forall env f_component fname args synthetic_body fdef,
    fn_env_unique_by_name env ->
    In f_component (env_fns env) ->
    callee_body_root_shadow_no_capture_direct_call_component_store_safe_summary
...
Proof.
  intros env f_component fname args synthetic_body fdef Hunique Hin_component
    Hcomponent Hcheck Htarget Hlookup.
  eapply component_body_no_capture_direct_call_component_target_in_of_closure_check;
    try eassumption.
  eapply check_fn_root_shadow_no_capture_direct_call_component_closure_of_exact_closure.
  exact Hcheck.
Qed.
          ...
          forall ftarget,
            ...
            callee_body_root_shadow_no_capture_direct_call_component_store_safe_summary
              (global_env_with_local_bounds
                ...
                (fn_bounds fcall)) ftarget.
Proof.
  intros env f_component Hunique Hin_component Hcomponent Hcheck_component
    fname_component args_component synthetic_component Htarget_component fdef
    Hlookup_component Hfdef_component fcall used used' fname args
    synthetic_body Hrename Htarget ftarget Hlookup_target.
  change (lookup_fn fname_component (env_fns env) = Some fdef)
    in Hlookup_component.
  change (lookup_fn fname (env_fns env) = Some ftarget) in Hlookup_target.
  pose proof (lookup_fn_b_of_lookup_fn fname_component (env_fns env) fdef
    Hlookup_component) as Hlookup_component_b.
  pose proof (lookup_fn_b_of_lookup_fn fname (env_fns env) ftarget
    Hlookup_target) as Hlookup_target_b.
  destruct (direct_call_target_expr_alpha_rename_fn_def_inv
              used fdef fcall used' fname args synthetic_body Hrename Htarget)
    as (args0 & Htarget_original).
  unfold check_fn_root_shadow_no_capture_direct_call_component_closure
    in Hcheck_component.
  destruct (check_fn_root_shadow_no_capture_direct_call_component_closure_seen_callee
              10001 [] env f_component fname_component args_component
              synthetic_component fdef Hcheck_component eq_refl
              Htarget_component Hlookup_component_b)
    as (fuel_component & Hfuel_component & Hcheck_fdef).
  inversion Hfuel_component; subst fuel_component; clear Hfuel_component.
  destruct (CheckerOrdinary.ident_in_b (fn_name fdef) [fn_name f_component])
    eqn:Hseen_fdef.
  - assert (Hsame_name : fn_name fdef = fn_name f_component).
    { simpl in Hseen_fdef.
      destruct (ident_eqb (fn_name fdef) (fn_name f_component)) eqn:Hname;
        try discriminate.
      apply ident_eqb_eq in Hname. exact Hname. }
    destruct (lookup_fn_in_name fname_component (env_fns env) fdef
      Hlookup_component) as [Hin_fdef _Hname_fdef].
    assert (Hsame_def : fdef = f_component).
    { eapply Hunique; eassumption. }
    subst fdef.
    eapply callee_body_root_shadow_no_capture_direct_call_component_store_safe_summary_global_env_with_local_bounds.
    ...
    + eapply component_body_no_capture_direct_call_component_target_in_of_closure_check.
      ...
      * exact Hin_component.
      * exact Hcomponent.
      * exact Hcheck_component.
      * exact Htarget_original.
      * exact Hlookup_target.
  - destruct (check_fn_root_shadow_no_capture_direct_call_component_closure_seen_callee
                10000 [fn_name f_component] env fdef fname args0
                (ECall fname args0) ftarget Hcheck_fdef Hseen_fdef
                Htarget_original Hlookup_target_b)
      as (fuel_target & _Hfuel_target & Hcheck_target).
    destruct (CheckerOrdinary.ident_in_b (fn_name ftarget)
                (fn_name fdef :: fn_name f_component :: []))
      eqn:Hseen_target.
    + simpl in Hseen_target.
      destruct (ident_eqb (fn_name ftarget) (fn_name fdef)) eqn:Htarget_fdef.
      * apply ident_eqb_eq in Htarget_fdef.
        destruct (lookup_fn_in_name fname_component (env_fns env) fdef
          Hlookup_component) as [Hin_fdef _].
        destruct (lookup_fn_in_name fname (env_fns env) ftarget
          Hlookup_target) as [Hin_target _].
        assert (ftarget = fdef) by (eapply Hunique; eassumption).
        subst ftarget.
        eapply callee_body_root_shadow_no_capture_direct_call_component_store_safe_summary_global_env_with_local_bounds.
        -- unfold fn_env_unique_by_name in *; simpl. exact Hunique.
        -- exact Hfdef_component.
      * destruct (ident_eqb (fn_name ftarget) (fn_name f_component))
          eqn:Htarget_component_name; try discriminate.
        apply ident_eqb_eq in Htarget_component_name.
        destruct (lookup_fn_in_name fname (env_fns env) ftarget
          Hlookup_target) as [Hin_target _].
        assert (ftarget = f_component) by (eapply Hunique; eassumption).
        subst ftarget.
        eapply callee_body_root_shadow_no_capture_direct_call_component_store_safe_summary_global_env_with_local_bounds.
        -- unfold fn_env_unique_by_name in *; simpl. exact Hunique.
        -- eapply callee_body_root_shadow_no_capture_direct_call_component_store_safe_summary_global_env_with_local_bounds.
...
Lemma component_body_no_capture_direct_call_component_alpha_nested_target_lookup_in_of_exact_closure_check :
  forall env f_component,
    fn_env_unique_by_name env ->
    In f_component (env_fns env) ->
    callee_body_root_shadow_no_capture_direct_call_component_store_safe_summary
...
Proof.
  intros env f_component Hunique Hin_component Hcomponent Hcheck_component.
  eapply component_body_no_capture_direct_call_component_alpha_nested_target_lookup_in_of_closure_check;
    try eassumption.
  eapply check_fn_root_shadow_no_capture_direct_call_component_closure_of_exact_closure.
  exact Hcheck_component.
Qed.

Lemma component_body_no_capture_direct_call_component_alpha_nested_target_lookup_in_provider_of_closure_check_provider :
  forall env,
    fn_env_unique_by_name env ->
    component_body_no_capture_direct_call_component_closure_check_provider env ->
    component_body_no_capture_direct_call_component_alpha_nested_target_lookup_in_provider env.
Proof.
  intros env Hunique Hprovider f_component Hin_component Hcomponent
    fname_component args_component synthetic_component Htarget_component fdef
    Hlookup_component Hfdef_component fcall used used' fname args
    synthetic_body Hrename Htarget ftarget Hlookup_target.
  change (lookup_fn fname_component (env_fns env) = Some fdef)
  ...
  destruct (direct_call_target_expr_alpha_rename_fn_def_inv
              used fdef fcall used' fname args synthetic_body Hrename Htarget)
    as (args0 & Htarget_original).
  pose proof (Hprovider f_component Hcomponent) as Hcheck_component.
  unfold check_fn_root_shadow_no_capture_direct_call_component_closure
    in Hcheck_component.
  destruct (check_fn_root_shadow_no_capture_direct_call_component_closure_seen_callee
              10001 [] env f_component fname_component args_component
...
Lemma check_fn_root_shadow_captured_call_store_safe_or_no_capture_direct_component_exact_closure_summary_sound :
  forall env fdef,
...
Proof.
  ...
  - right.
    destruct (check_fn_root_shadow_no_capture_direct_call_component_exact_closure_head_sound
                env fdef Hclosure) as [Hcomponent _].
    exact Hcomponent.
Qed.

Lemma check_env_root_shadow_captured_call_store_safe_or_no_capture_direct_component_exact_closure_summary_ready :
  forall env,
...
Lemma check_fn_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary_strict_sound :
  forall env fdef,
...
Lemma check_env_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary_strict_ready :
  forall env,
...
Proof.
  ...
  destruct (lookup_fn_in_name fname (env_fns env) fdef Hlookup)
    as [Hin _].
  apply forallb_forall with (x := fdef) in Hcheck; [| exact Hin].
  apply check_fn_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary_strict_sound.
  exact Hcheck.
Qed.

Lemma check_fn_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary_sound :
  forall env fdef,
    check_fn_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary
      env fdef = true ->
    callee_body_root_shadow_captured_call_store_safe_or_no_capture_direct_component_summary
...
Lemma check_env_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary_ready :
  forall env,
    check_env_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary
      env = true ->
    env_fns_root_shadow_captured_call_store_safe_or_no_capture_direct_component_summary_ready
...
Proof.
  ...
  destruct (lookup_fn_in_name fname (env_fns env) fdef Hlookup)
    as [Hin _].
  apply forallb_forall with (x := fdef) in Hcheck; [| exact Hin].
  apply check_fn_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary_sound.
  exact Hcheck.
Qed.

Lemma check_fn_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary_component_exact_closure :
  forall env fdef,
    check_fn_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary
      env fdef = true ->
    check_fn_root_shadow_no_capture_direct_call_component_store_safe_summary
    ...
    check_fn_root_shadow_no_capture_direct_call_component_exact_closure
      env fdef = true.
Proof.
  intros env fdef Hcheck Hcomponent.
  unfold check_fn_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary
    in Hcheck.
  rewrite Hcomponent in Hcheck.
  exact Hcheck.
Qed.

Lemma check_fn_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary_component_check_when_not_captured :
  forall env fdef,
    check_fn_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary
      env fdef = true ->
    check_fn_root_shadow_captured_call_store_safe_summary env fdef = false ->
...
Proof.
  ...
  unfold check_fn_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary
    in Hstrict.
  destruct (check_fn_root_shadow_no_capture_direct_call_component_store_safe_summary
              env fdef) eqn:Hcomponent_check; try reflexivity.
...
Lemma check_fn_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary_component_ready :
  forall env fdef,
    ...
    check_fn_root_shadow_no_capture_direct_call_component_exact_closure
      env fdef = true.
Proof.
  intros env fdef Hstrict Hcomponent_check.
  split.
  - eapply check_fn_root_shadow_no_capture_direct_call_component_store_safe_summary_sound.
    exact Hcomponent_check.
...
Lemma check_env_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary_component_check_when_not_captured :
  forall env fdef,
    check_env_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary
      env = true ->
    In fdef (env_fns env) ->
...
Proof.
  ...
  unfold check_env_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary
    in Hcheck.
  apply forallb_forall with (x := fdef) in Hcheck; [| exact Hin].
  eapply check_fn_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary_component_check_when_not_captured;
    eassumption.
Qed.
...
Lemma check_env_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary_component_ready :
  forall env f_component,
    ...
    check_fn_root_shadow_no_capture_direct_call_component_exact_closure
      env f_component = true.
Proof.
  intros env f_component Hcheck Hin Hcomponent_check.
  unfold check_env_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary
    in Hcheck.
  apply forallb_forall with (x := f_component) in Hcheck; [| exact Hin].
  eapply check_fn_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary_component_ready;
    eassumption.
Qed.

Lemma check_env_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary_component_exact_closure :
  forall env f_component,
    check_env_root_shadow_strict_exact_closure_captured_or_no_capture_direct_component_summary
      env = true ->
    In f_component (env_fns env) ->
    ...
    check_fn_root_shadow_no_capture_direct_call_component_exact_closure
      env f_component = true.
Proof.
  intros env f_component Hcheck Hin Hcomponent.
