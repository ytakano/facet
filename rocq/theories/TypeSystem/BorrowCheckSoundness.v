...
Proof.
  ...
  + apply BO_Enum.
    apply borrow_check_call_go in Hcheck.
    revert BS BS' Hcheck.
    induction l2 as [| a rest IHargs]; intros BS BS' Hcheck.
    * simpl in Hcheck. injection Hcheck as <-. constructor.
    * simpl in Hcheck.
      destruct (borrow_check fenv BS Γ a) as [BS1|] eqn:Ha; [|discriminate].
      apply BO_Args_Cons with (BS1 := BS1).
  ...
  + simpl in Hcheck.
    ...
    * destruct (bs_has_mut r BS) eqn:Hmut; [discriminate|].
      apply BO_Deref.
      -- unfold borrow_ok_deref_check. rewrite Hroot.
         unfold bs_can_shared. exact Hmut.
      -- apply IH with (e := e). simpl in Hlt; lia. exact Hcheck.
    * apply BO_Deref.
      -- unfold borrow_ok_deref_check. rewrite Hroot. exact I.
      -- apply IH with (e := e). simpl in Hlt; lia. exact Hcheck.
...
Proof.
  ...
  - intros BS BS' Γ e Hfreeze _ IH.
    simpl.
    unfold borrow_ok_deref_check in Hfreeze.
    destruct (expr_ref_root e) as [r |].
    + unfold bs_can_shared in Hfreeze. rewrite Hfreeze. exact IH.
    + exact IH.
  ...
  (* BO_Enum *)
  - intros BS BS' Γ enum_name variant_name lts variant_lts args payloads _ IHargs.
    simpl.
    apply borrow_check_call_go.
    exact IHargs.
