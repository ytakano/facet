...
Last updated implementation point: call runtime semantics now freshens callee function bodies against caller/captured store names before binding parameters, using shared `Renaming.v` definitions. Freshened parameter names are proved fresh/no-duplicate for `bind_params`, and alpha-renamed parameter contexts are connected to `ctx_alpha`. Forward alpha-renaming for structural typing is implemented, including the `typed_env_structural` theorem and a function-level bridge from `alpha_rename_fn_def` to `typed_fn_env_structural` under original parameter `NoDup`. The latest TypeSafety slice adds `env_fns_preservation_ready` and a direct-place `place_path` alpha-renaming helper. Direct `PRE_Call` / `ECall` preservation was deliberately not enabled: adding call readiness to the existing ready predicate would break `eval_preserves_typing_ready_mutual` until readiness is proved stable under alpha-renaming and call preservation is stated as a separate or strengthened theorem. Checked structural/root invariants now carry `NoDup (ctx_names (params_ctx (fn_params f)))`, with lookup helpers exposed for future call preservation. Runtime aliasing correspondence and full call preservation remain future work.
...
4. **S3: function call / current closure value safety** `[todo]`
   ...
   - `[todo]` alpha-renaming が readiness を保存することを証明する。
   - `[todo]` 既存の ready predicate に call readiness を直接追加すると `eval_preserves_typing_ready_mutual` が壊れるため、`PRE_Call` を有効化する前に separate / strengthened call-ready preservation theorem を導入する。
...
23. `[partial]` freshened callee body typing を `typed_env_structural` へ移す方針は global alpha-renaming forward theorem に固定済み。forward `ctx_alpha`/typed-place/args/fields/`expr_as_place` helper、`typed_env_structural` 本体の forward theorem、function-level bridge、`env_fns_preservation_ready`、direct-place `place_path` alpha-renaming helper は追加済み。direct `PRE_Call` / `ECall` preservation は未追加。次は alpha-renaming が readiness を保存することを証明し、既存 ready mutual theorem を壊さない separate / strengthened call-ready preservation theorem を導入してから `PRE_Call` を有効化する。
