...
- The mixed endpoint avoids the final direct-receiver gate for programs
  ...
  from assoc-base mixed success. Retargeting the public theorem directly through
  the assoc-base mixed case-split still needs a valid source of route-local
  evidence-at or non-captured facts for the no-direct-ready branch; the public
  prefix route alone requires global callee evidence and is not sufficient.
- Verification note: `rocq compile -R theories Facet
  ...
  component-summary variant and the non-captured-only mixed callback wrappers.
