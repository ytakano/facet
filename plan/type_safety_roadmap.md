...
23. `[partial]` freshened callee body typing を `typed_env_structural` へ移す方針は global alpha-renaming forward theorem に固定済み。forward `ctx_alpha`/typed-place/args/fields/`expr_as_place` helper、`typed_env_structural` 本体の forward theorem、function-level bridge は追加済み。checked structural/root invariants は original parameter `NoDup` を保持し、lookup helpers で取り出せる。次は full `ECall` / `ECallExpr` preservation へ進む。
